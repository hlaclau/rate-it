...
<template>
  <UContainer class="py-10">
    <div class="flex flex-wrap items-center justify-between gap-4 mb-8">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <div class="flex gap-2">
          ...
          ]" :key="f.value" :variant="activeFilter === f.value ? 'solid' : 'ghost'" color="neutral" size="sm"
            @click="activeFilter = (f.value as 'all' | 'watched' | 'plan_to_watch')">
            {{ f.label }}
          </UButton>
        </div>
        <USelect v-model="sortValue" :items="sortOptions" value-key="value" label-key="label" size="sm" class="w-52" />
      </div>
    </div>

    <!-- Loading -->
    <div v-if="listLoading" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
      <USkeleton v-for="i in 10" :key="i" class="aspect-[2/3] rounded-xl" />
    </div>

    <!-- Empty state -->
    <div v-else-if="filteredList.length === 0" class="flex flex-col items-center gap-4 py-24 text-center">
    ...
    <div v-else class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
      <div v-for="entry in filteredList" :key="entry.media_id"
        class="group relative rounded-xl overflow-hidden bg-elevated ring-1 ring-default hover:ring-primary transition-all">
        <NuxtLink :to="entry.type === 'series'
          ? `/series/${entry.external_id}`
          : `/movie/${entry.external_id}`
          ">
          <img v-if="posterUrl(entry.poster_path)" :src="posterUrl(entry.poster_path)!" :alt="entry.title"
            class="w-full aspect-[2/3] object-cover" />
          <div v-else class="w-full aspect-[2/3] bg-muted flex items-center justify-center">
        ...
        <div class="p-3 space-y-1.5">
          ...
          <div class="flex items-center justify-between gap-2">
            <UBadge :color="entry.status === 'watched' ? 'success' : 'info'" size="xs">
              {{ entry.status === 'watched' ? 'Watched' : 'Plan to watch' }}
            </UBadge>
            <span v-if="entry.rating" class="flex items-center gap-0.5 text-xs font-medium shrink-0">
        ...
        <UButton icon="i-lucide-x" size="xs" color="neutral" variant="solid"
    ...
    <UModal :open="!!pendingRemove" title="Remove from list" :description="pendingRemove
